...
final class SvgIconProviderTest extends FunctionalTestCase
{
    ...
    {
        // TODO this can be simplified once support for v13+v14 is dropped
        $this->svgIconProvider->prepareIconMarkup($this->icon, ['source' => 'typo3.svg']);
        $expectedSvgMarkup = [
        ...
        $result = $this->subject->getIcon($this->registeredIconIdentifier)->getAlternativeMarkup(SvgIconProvider::MARKUP_IDENTIFIER_INLINE);
        foreach ($expectedSvgMarkup as $part) {
            self::assertStringContainsString($part, $result);
        }
    }
